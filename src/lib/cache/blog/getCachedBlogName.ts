...
export const getCachedBlogName = cache(
  async (blogName: string): Promise<CachedBlog> => {
    const getCachedData = unstable_cache(
      async (blogName: string) => {
        console.log(`Fetching blog post ${blogName}`);
        return (
          (
            await db
              .select({
      ...
      {
        revalidate: 60 * 60 * 24,
        tags: [`blogPostTag${blogName}`, "signInState"],
      },
    );
